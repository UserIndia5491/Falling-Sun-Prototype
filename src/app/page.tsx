...
export default function Home() {
  ...
  return (
    <main className="brutalist-page min-h-screen">
      ...
      <section className="section people-section"><div className="page-width"><SectionTitle index="07" light>The crew<br /><span className="yellow-text">behind it.</span></SectionTitle><p className="crew-intro">7 humans, one sun. No corporate fluff — just builders who ship. Brutalist by design, brutal when we need to be.</p><div className="crew-grid">{organizers.map((person, i) => <motion.article key={person.label} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }} viewport={{ once: true }} className="crew-card"><div className="crew-photo"><img src={person.image} alt={person.imageAlt || person.label} /><span className="crew-photo-badge">CORE // 0{i + 1}</span><span className="crew-photo-index">{String(i + 1).padStart(2, "0")}</span></div><div className="crew-body"><h3>{person.label}</h3><p>Organizer — Falling Sun</p><p className="crew-bio">Shipping the show, unblocking builders, keeping the 24h chaos crisp.</p></div></motion.article>)}</div><div className="mentor-divider">MENTORS // ON-DECK <span>2</span></div><div className="mentor-grid-brutal">{mentors.map(mentor => <div key={mentor.name} className="mentor-card-brutal"><img src={mentor.image} alt={mentor.name} /><div><h3>{mentor.name}</h3><p>{mentor.role} / READY TO HELP</p><span className="mentor-tag">ASK ME ANYTHING →</span></div></div>)}</div></div></section>
