...
export default function Home() {
  ...
  return (
    <main className="brutalist-page min-h-screen">
      ...
      <section id="faq" className="section page-width faq-section"><SectionTitle index="09">Questions?<br /><span className="red-text">Decoded.</span></SectionTitle><p className="section-intro" style={{marginTop: -22}}>9 answers — tap to decode.</p><Accordion type="single" collapsible className="faq-list faq-grid">{faq.map(([question, answer], i) => <AccordionItem key={question} value={`item-${i}`} className="faq-item"><AccordionTrigger>{question}</AccordionTrigger><AccordionContent>{answer}</AccordionContent></AccordionItem>)}</Accordion></section>
